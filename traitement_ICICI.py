...
def exctract_champ_ICICI(max_loc, path, image):
    n, m, _ = image.shape
    pay_champ = image[max_loc[1]+180*n//1093:max_loc[1]+280 *
                      n//1093, max_loc[0]+170*m//2387:max_loc[0]+1800*m//2387]
    gray = cv2.cvtColor(pay_champ, cv2.COLOR_BGR2GRAY)
    pay_champ = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY)[1]

    rupees_champ = image[max_loc[1]+280*n//1093:max_loc[1]+400 *
                         n//1093, max_loc[0]+170*m//2387:max_loc[0]+1800*m//2387]
    gray = cv2.cvtColor(rupees_champ, cv2.COLOR_BGR2GRAY)
    rupees_champ = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY)[1]

    date_champ = image[max_loc[1]+40*n//1093:max_loc[1]+130 *
                       n//1093, max_loc[0]+1650*m//2387:max_loc[0]+2200*m//2387]
    ...
    montant_champ = image[max_loc[1]+330*n//1093:max_loc[1]+500 *
                          n//1093, max_loc[1]+1650*m//2387:max_loc[1]+2400*m//2387]
    gray = cv2.cvtColor(montant_champ, cv2.COLOR_BGR2GRAY)
    montant_champ = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY)[1]
    AC_champ = image[max_loc[1]+490*n//1093:max_loc[1]+600*n //
                     1093, max_loc[0]+200*m//2387:max_loc[0]+700*m//2387]
    ...
    sign_champ = image[max_loc[1]+500*n//1093:max_loc[1] +
                       880*n//1093, -700*m//2387:-70*m//2387]
